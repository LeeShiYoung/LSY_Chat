...
class FTChatMessageModel: NSObject {
    ...
    convenience init(data : AnyObject? ,time : String?, from : FTChatMessageSenderModel, type : FTChatMessageType){
        self.init()
...
//        self.transformMessage(data,time : time,extraDic: nil,from: from,type: type)
    ...
    private func transformMessage(data : AnyObject? ,time : String?, extraDic : NSDictionary?, from : FTChatMessageSenderModel, type : FTChatMessageType){
        messageType = type
        messageObject = data
