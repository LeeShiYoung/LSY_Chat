...
class FTChatMessageBubbleTextItem: FTChatMessageBubbleItem {
    ...
    convenience init(frame: CGRect, aMessage : FTChatMessageModel, messageLabel: UILabel) {
...
//        messageLabel = UILabel(frame: self.getTextRectWithSize(frame.size, isUserSelf: aMessage.isUserSelf));
        messageLabel.frame = getTextRectWithSize(frame.size, isUserSelf: aMessage.isUserSelf)
        messageLabel.text = message.messageObject as? String
